...
#' @examples
#'
#' scale_001_inv2 <- fit_grm2_inv2(
#' scale_info = scales_data,
#' scale_num  = 1,
#' data = data_model)
#'
#'
fit_grm2_inv2 <- function(data, scale_num, scale_info) {
...
first_line <- '\n'
file_line <- paste0('FILE = ',mplus_file,'_inv2_scores.dat;\n')
save_line <- 'SAVE = FSCORES;\n'
save_h5_line <- paste0('H5RESULTS = ',mplus_file,'__inv2_results.H5;\n')
...
mplus_object <- grm_model %>%
                ...
                mplusModeler(.,
                modelout = paste0(mplus_file,'_inv2.inp'),
                run = 1L,
                hashfilename = FALSE,
                writeData = 'always')
