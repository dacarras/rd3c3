...
#'
#'
fit_grm2_m02_scalar <- function(data, scale_num, scale_info, grp_var, grp_txt, grp_ref) {
...
first_line <- '\n'
file_line <- paste0('FILE = ',mplus_file,'_inv_02_scores.dat;\n')
...
model_structure <- gen_scalar_model(grp_lst, grp_ref, var_names, thresholds, file = paste0(mplus_file,'_inv_02_mod.txt'))
...
mplus_object <- grm_model %>%
                ...
                mplusModeler(.,
                modelout = paste0(mplus_file,'_inv_02.inp'),
                run = 1L,
                hashfilename = FALSE,
                writeData = 'always')
