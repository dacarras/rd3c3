...
#'
#'
fit_grm2_m01_strict <- function(data, scale_num, scale_info, grp_var, grp_txt, grp_ref) {
...
model_structure <- gen_strict_model(grp_lst, grp_ref, var_names, thresholds, file = paste0(mplus_file,'_inv_01_mod.txt'))
...
mplus_object <- grm_model %>%
                ...
                mplusModeler(.,
                modelout = paste0(mplus_file,'_inv_01.inp'),
                run = 1L,
                hashfilename = FALSE,
                writeData = 'always')
