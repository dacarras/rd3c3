...
#' @examples
#'
#' scale_001_inv1 <- fit_grm2_inv1(
#' scale_info = scales_data,
#' scale_num  = 1,
#' data = data_model)
#'
#'
fit_grm2_inv1 <- function(data, scale_num, scale_info) {
...
first_line <- '\n'
file_line <- paste0('FILE = ',mplus_file,'_inv1_scores.dat;\n')
save_line <- 'SAVE = FSCORES;\n'
save_h5_line <- paste0('H5RESULTS = ',mplus_file,'__inv1_results.H5;\n')
...
mplus_object <- grm_model %>%
                ...
                mplusModeler(.,
                modelout = paste0(mplus_file,'_inv1.inp'),
                run = 1L,
                hashfilename = FALSE,
                writeData = 'always')
